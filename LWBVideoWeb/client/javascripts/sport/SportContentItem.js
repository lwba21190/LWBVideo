...
import SportContentLargeItem from './SportContentLargeItem';
...
class SportContentItem extends Component{
            ...
            <ul style={{display:'flex',width:width,height:height-40,justifyContent:'space-between',flexWrap:'wrap'}}>
                {
                    info.map(function(value,index,array){
                        if(index == 0){
                            return (
                                <li style={{display:'inline-block',backgroundColor:'#cccccc'}} key={index}><SportContentLargeItem info={value} width={smallItemWidth*2+widthSpacing} height={smallItemHeight}/></li>
                            );
                        }
                        return (
...
export default SportContentItem;